...
public class GeometryDecoder {
    ...
    public static GeometryColumn decodeGeometryColumn(byte[] tile, int numStreams, IntWrapper offset) {
        var geometryTypeMetadata = StreamMetadata.decode(tile, offset);
        ...
        for(var i = 0; i < numStreams - 1; i++){
            var geometryStreamMetadata = StreamMetadata.decode(tile, offset);
            switch (geometryStreamMetadata.streamType()){
                case NUM_GEOMETRIES:
                ...
                case VERTEX_BUFFER:
                    ...
                    break;
                case MORTON_VERTEX_BUFFER:
                    mortonVertexBuffer = IntegerDecoder.decodeMortonStream(tile, offset, geometryStreamMetadata);
                    break;
            }
        }

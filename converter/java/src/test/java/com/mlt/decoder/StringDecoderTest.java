...
public class StringDecoderTest {
    ...
    public void decodeSharedDictionary_FsstDictionaryEncoded() throws IOException {
        ...
        var tileMetadata = MaplibreTileMetadata.FieldMetadata.newBuilder().addChildren(
                MaplibreTileMetadata.FieldMetadata.newBuilder().setName("Test").setDataType(MaplibreTileMetadata.DataType.STRING).build()
        ).addChildren(
                MaplibreTileMetadata.FieldMetadata.newBuilder().setName("Test2").setDataType(MaplibreTileMetadata.DataType.STRING).build()
        ).build();
    ...
    public void decodeSharedDictionary_MvtWithNestedColumns() throws IOException {
        ...
        var fieldMetadata = tileMetadata.getLayers(0).getFieldsList().stream().
                filter(f -> f.getName().equals("name")).findFirst().get();

        var layer = mvTile.layers().get(0);
        var sharedValues = new ArrayList<List<String>>();
        for(var column : fieldMetadata.getChildrenList()){
            var values = new ArrayList<String>();
            for(var feature : layer.features()){
    ...
    public void decodeSharedDictionary_MvtWithNestedColumns2() throws IOException {
        ...
        var columnMapping = new ColumnMapping("name", ":", true);
        var tileMetadata = MltConverter.createTileMetadata(mvTile, Optional.of(List.of(columnMapping)),true);
        var fieldMetadata = tileMetadata.getLayers(3).getFieldsList().stream().
                filter(f -> f.getName().equals("name")).findFirst().get();
